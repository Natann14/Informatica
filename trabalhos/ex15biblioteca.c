...
void imprime_tela(wins_t *pwin)
{
    ...
    mvwhline(pwin->w[pwin->wi], 2, 1,'_',COLS-2);             /* linha separadora abaixo do titulo */
    mvwprintw(pwin->w[pwin->wi], 3, 1,"%s", stela[pwin->wi]); /* o numero e o nome da tela atual */
    mvwhline(pwin->w[pwin->wi], 4, 1,'_',COLS-2);             /* linha separadora abaixo do nome da tela */
    mvwhline(pwin->w[pwin->wi], STLINE-1, 1,'_',COLS-2);      /* linha separadora acima da linha status */
    mvwprintw(pwin->w[pwin->wi], STLINE, 1,"%s", TITULOST);   /* titulo da linha status */
    mvwhline(pwin->w[pwin->wi], STLINE+1, 1,'_',COLS-2);      /* linha separadora abaixo da linha status */

    /* linha indicadora de funcoes das teclas, mostradas abaixo da mensagem de status */
    wattrset(pwin->w[pwin->wi], A_REVERSE);
    mvwprintw(pwin->w[pwin->wi], STLINE+2, centralisar(COLS,"   TECLAS:   |   F1:HELP   |   F2:CADASTRO   |   F3:PESQUISA   |   F4:SALVAR   |   F8:SAIR   |" ),"   TECLAS:   |   F1:HELP   |   F2:CADASTRO   |   F3:PESQUISA   |   F4:SALVAR   |   F8:SAIR   |");
...
void funcao_help(wins_t *pwin)
{
    ...
    mvwprintw(pwin->w[pwin->wi],12, 1, "<F8>         : Saida segura do programa.");
    mvwprintw(pwin->w[pwin->wi],13, 1, "<Q>          : Saida forcada do programa.");

    mvwhline(pwin->w[pwin->wi], 16, 1,'_',COLS-2); /* Linha de separacao da tela */

    /*imprime as outras instruções na tela help*/
    wattron(pwin->w[pwin->wi], A_STANDOUT);
    ...
    mvwprintw(pwin->w[pwin->wi],23, 1, "==> Complete, pelo menos, os campos obrigatorios (5 primeiros) e salve utilizando a tecla <F4>.");
    wattron(pwin->w[pwin->wi], A_UNDERLINE);
    mvwprintw(pwin->w[pwin->wi],25, 1, "*Pesquisa de Livros*");
    wattroff(pwin->w[pwin->wi], A_UNDERLINE);
    mvwprintw(pwin->w[pwin->wi],26, 1, "==> Complete os campos necessarios e busque-os utilizando a tecla <F4>. Letras maiusculas ou minusculas serao aceitas e encontradas.");
    mvwprintw(pwin->w[pwin->wi],27, 1, "    Apos isso, escolha entre as opcoes de pesquisa 'OR' ou 'AND' na barra de status.");
    wattron(pwin->w[pwin->wi], A_UNDERLINE);
    mvwprintw(pwin->w[pwin->wi],29, 1, "*Retificacao de Livros*");
    wattroff(pwin->w[pwin->wi], A_UNDERLINE);
    mvwprintw(pwin->w[pwin->wi],30, 1, "==> Primeiramente deve acessar a janela 'pesquisa de livros' (<F3>), selecionar um livro e a opcao 'alterar' na barra de status.");
    mvwprintw(pwin->w[pwin->wi],31, 1, "    Apos isso, todos os campos desejaveis poderao ser retificados de forma similar ao cadastro de livros.");
    wattron(pwin->w[pwin->wi], A_UNDERLINE);
    mvwprintw(pwin->w[pwin->wi],33, 1, "*Remocao de Livros*");
    wattroff(pwin->w[pwin->wi], A_UNDERLINE);
    mvwprintw(pwin->w[pwin->wi],34, 1, "==> Primeiramente acesse a janela 'pesquisa de livros' (<F3>), selecione um livro e a opcao 'deletar' na barra de status. Apos isso, o livro sera deletado.");
    mvwprintw(pwin->w[pwin->wi],36, 1, "OBSERVACAO: Caso qualquer erro tenha sido cometido ao completar um ou mais campos no momento de cadastrar ou retificar um livro,");
    mvwprintw(pwin->w[pwin->wi],37, 1, "            o usuario deve selecionar a opcao 'fechar' e selecionar a janela novamente, se assim desejar.");
...
void funcaoAdd(wins_t *pwin)
{
    ...
    fclose(file);
      ...
      memset(autorC,0,strlen(autorC)); // prepara as memorias para o input zerando as
      ...
      mvwhline(pwin->w[pwin->wi], 15, 1,'_',COLS-2); /* Linha de separacao da tela  */
...
if(mensagem_status(*pwin, "Deseja adicionar um livro?", YESNO, "SIM", "NAO")) // verifica caso o usuario queira ou nao entrar na fun add
    {
    ...
    while(1)
    {
        ...
        for(r=0;r<=8;r++) // loop para gerar o menu com as opcoes
        {
            ...
            refresh();
                ...
                switch(choice){
                case KEY_UP:               // interpreta o input movimentando o cursor
                    select--;
                    if(select == -1){
                        select = 7;
                    }
        ...
        if(choice == 10)
        {
            if(select == 0){      /*se o usuario escolher autor*/
            ...
            if(select == 1){   /*se o usuario escolher titulo*/
                 for(r = 0; r < 200; r++){
                    ...
                    refresh();
                        ...
                        if(titulo[r] == ';')  /*caso o usuario digite ';'*/
                                                {
                                 if(mensagem_status(*pwin, "Nao pode haver ';', favor digite novamente", STOK, "Ok", NULL))
                        {
            ...
            if(select == 2){    /* caso o usuario escolha ano*/
                 for(r = 0; r < 10; r++){
            ...
            if(select == 3){           /*caso o usuario escolha editora*/
                 for(r = 0; r < 100; r++){
            ...
            if(select == 4){        /*caso o usuario escolha local*/
                 for(r = 0; r < 100; r++){
             ...
             if(select == 5){     /*caso o usuario escolha edicao*/
                 for(r = 0; r < 100; r++){
            ...
            if(select == 6){    /*caso o usuario escolha ISBN*/
                 for(r = 0; r < 20; r++){
           ...
           if(select == 7){
                if(mensagem_status(*pwin, "Certeza que quer perder as alteracoes? ", YESNO, "SIM" , "NAO")) /* Verifica se o usuario quer ou nao sair da fun*/
                {
                    pwin->wi = 0;
                    funcao_help(pwin);
                    break;
        ...
        {
            if(autorC[1] != 0) /* verifica se ha informaocoes nas memorias*/
        {
          o++;
        }
        if(tituloC[1] != 0)
        ...
        if(anoC[1] != 0)
        {
          o++;
                ...
                if(o>=5)    /*caso todos as infos estejam preenchidas */
        {
            if(mensagem_status(*pwin, "Deseja salvar as alteracoes? ", YESNO, "SIM" , "NAO"))  //mensagem que a barra status ira imprimir e pegar o input
                  ...
                  refresh();
                    ...
                    break;
                        ...
                        if(o<5)      /* Caso nao estajam preechidas*/
        {
          o=0;
...
void funcaoBuscar(wins_t *pwin)
{
    ...
    mvwprintw(pwin->w[pwin->wi], 16, COLS/2 - 90 ,  "(OR / AND):");
    mvwprintw(pwin->w[pwin->wi], 18, COLS/2 - 90,  "OR -> Busca por livro(s) que possua(m) QUAISQUER campos inseridos semelhantes");
    mvwprintw(pwin->w[pwin->wi], 19, COLS/2 - 91, "       aos dos livros presentes no banco de dados.");
    mvwprintw(pwin->w[pwin->wi], 21, COLS/2 - 90, "AND -> Busca por livro(s) que possua(m) APENAS campos inseridos semelhantes");
    mvwprintw(pwin->w[pwin->wi], 22, COLS/2 - 91, "       aos dos livros presentes banco de dados.");
    mvwprintw(pwin->w[pwin->wi], 16, COLS/2, " TECLAS IMPORTANTES:  ");
    mvwprintw(pwin->w[pwin->wi], 18, COLS/2, " <F3> -> Caso nenhum livro seja encontrado, inicia busca novamente.");
    mvwprintw(pwin->w[pwin->wi], 20, COLS/2, " <F8> -> Caso o livro desejado nao esteja disponivel na lista, volta para busca.");
    wrefresh(pwin->w[pwin->wi]);
    refresh();
    ...
    file = fopen("livros.txt","r");     /*abre o arquivo livros.txt*/
    if(file == NULL)                   /*caso nao encontre o arquivo*/
    {
        mvwprintw(pwin->w[pwin->wi], 11, 1,"ERRO arquivo não encontrado");
        exit(0);
    }
    keypad(pwin->w[pwin->wi], true);
    mvwhline(pwin->w[pwin->wi], LINES/2, 1,'_',COLS-2);

    if(mensagem_status(*pwin, "Deseja pesquisar um livro?", YESNO, "SIM", "NAO"))
    {

    while(1)
    {
        for(int j = 0; j<8 ; j++ ) /*criação do menu para saber o livro que o usuário vai procurar*/
            if(j == contador)
            {
                wattron(pwin->w[pwin->wi], A_REVERSE);
        ...
        switch(kpress)                   //analisa o que o usuário pessiona
        {
            case KEY_DOWN:               //desce as opções
            {
                contador ++;
                if(contador == 8)
                    contador = 7;
        ...
        if(kpress == '\n')
        {
            if(contador == 0)    //se o usuário escolher autor
            {
                for(r = 0; r < 200; r++)
                {
                    autores[r] = getch();
            ...
            if(contador == 1)   //se o usuário escolher título
            {
                for(r = 0; r < 200; r++)
                {
                    titulo[r] = getch();
            ...
            if(contador == 2)        //se o usuário escolhereditora
            {
                for(r = 0; r < 100; r++)
                {
                    editora[r] = getch();
            ...
            if(contador == 3)          //se o usuário escolher local
            {
                for(r = 0; r < 100; r++)
                {
                    local[r] = getch();
            ...
            if(contador == 4)     //se o usuário escolher ano
            {
                for(r = 0; r < 10; r++)
                {
                    ano[r] = getch();
            ...
            if(contador == 5)      //se o usuário escolher edição
            {
                for(r = 0; r < 10; r++)
                {
                    edicao[r] = getch();
            ...
            if(contador == 6)      //se o usuário escolher ISBN
            {
                for(r = 0; r < 20; r++)
                {
                    ISBN[r] = getch();
        ...
        if(kpress == KEY_F(4))       //se o usuário apertar f4 ele cria a string que será comparada no arquivo livros.txt
        {
            ...
            wrefresh(pwin->w[pwin->wi]);
             ...
             /*chama a função status para o usuário selecionar se quer pesquisar com "ou" ou "and"*/
            ...
            while(fgets(livro,626,file) != NULL)
            {
                ...
                lixo = strsep(&base ,";");  //procura o livro procurado no arquivo a partir da ";"
            ...
            if(escolha)  //se  escolher "OR"
                mvwprintw(pwin->w[pwin->wi], (LINES/2)+ 1 , 1, "Busca por conectivo OR | <- -> move as abas");
            else         //se escolher "AND"
                mvwprintw(pwin->w[pwin->wi], (LINES/2)+ 1 , 1, "Busca por conectivo  AND | <- -> move as abas");
            mvwprintw(pwin->w[pwin->wi], (LINES/2)+ 2 , 1, "Livros encontrados: %i", i);  //mostra os livros encontrados no arquivo .txt
            wrefresh(pwin->w[pwin->wi]);
            refresh();
            if(i)
            {
                do{              /* limpa as opçoes */
                    ...
                    if((i - aba) > 10)            //se tiver mais que 10 opçoes imprime apenas 10
                        manipular_aba = 10;
                    else
                    {
                        manipular_aba = (i - aba); // se tiver menos que 10 imprime apenas a quantidade que tiver
                        if(quantidade > (manipular_aba - 1))
                            quantidade = manipular_aba - 1;
                    }
                    for(int j = 0; j<manipular_aba ; j++ )  //imprime opção na cor inverversa
                        if(j == quantidade)
                        {
                            wattron(pwin->w[pwin->wi], A_REVERSE);
                            imrpimir_opcao(pwin, lista[ j + aba], j);
                            wattroff(pwin->w[pwin->wi], A_REVERSE);
                        }
                        else
                            imrpimir_opcao(pwin, lista[j+aba], j);
                    ...
                    switch(kpress)
                    {
                        case KEY_RIGHT:     //imprime as 10 seguinte
                        {
                            if((aba + 10) < i)
                                aba +=10;
                            break;
                        }
                        case KEY_LEFT:     //imprime as 10 anteriores
                        {
                            aba -= 10;
                            if(aba < 0)
                                aba=0;
                        ...
                        case KEY_DOWN:     //desce a opção
                        {
                            ...
                            {
                                if(quantidade >= manipular_aba)   // em caso da opção estiver menor que a quantidade
                                    quantidade = manipular_aba - 1;
                            }
                            break;
                        }
                        case KEY_UP:     // sobe a opção
                        {
                            quantidade --;
                            if(quantidade == -1)
                                quantidade = 0;
                ...
                imrpimir_opcao(pwin, lista[ quantidade + aba], -(LINES/2)-4+(STLINE - 2));
                wrefresh(pwin->w[pwin->wi]);

                /*chama a função status para o usuário escolher se quer editar ou deletar o livro*/

                escolha = mensagem_status(*pwin, "Opcoes :", YESNO, "ALTERAR", "DELETAR");
                if(escolha == 1) /* alterar */
                {
                    pwin->wi = 3;
                    funcaoalterar(pwin, lista[quantidade]);  //chama a função alterar
                }
                if(escolha == 0) /* deletar */
                {
                    pwin->wi = 4;
                    funcaodeletar(pwin, lista[quantidade]);   //chama a função deletar
                }
            }
                    break;
...
/* --------------------------------------------------------------------------- */
/* funcao responsavel por imprimir as opcoes achadas da tela busca*/
void imrpimir_opcao(wins_t *pwin, char livro[626], int j)
{
    char *token; /* fragmento da linha do livro*/
    char *base;  /* copia do livro recebido */
    int tamanho = 0; /* tamanho da string */
    base = strdup(livro);


    token = strsep(&base ,";");
    if(token && (strcmp(token,"\0") > 0)) /* se tiver codigo imprime*/
    {
        mvwprintw(pwin->w[pwin->wi], (LINES/2) + 4 + j, 1, "[%s]",token);
        tamanho += strlen(token) + 2;
    }
    token = strsep(&base ,";");
    if(token && (strcmp(token,"\0") > 0)) /* se tiver autor imprime*/
    {
        mvwprintw(pwin->w[pwin->wi], (LINES/2) + 4 + j, 1 + tamanho, ", %s",token);
        tamanho += strlen(token) + 2;
    }
    token = strsep(&base ,";");
    if(token && (strcmp(token,"\0") > 0))/* se tiver titulo imprime*/
    {
        mvwprintw(pwin->w[pwin->wi], (LINES/2) + 4 + j, 1 + tamanho, ", %s",token);
        tamanho += strlen(token) + 2;
    }
    token = strsep(&base ,";");
    if(token && (strcmp(token,"\0") > 0))/* se tiver editora imprime*/
    {
        mvwprintw(pwin->w[pwin->wi], (LINES/2) + 4 + j, 1 + tamanho, ", %s",token);
        tamanho += strlen(token) + 2;
    }
    token = strsep(&base ,";");
    if(token && (strcmp(token,"\0") > 0))/* se tiver local imprime*/
    {
        mvwprintw(pwin->w[pwin->wi], (LINES/2) + 4 + j, 1 + tamanho, ", %s",token);
        tamanho += strlen(token) + 2;
    }
    token = strsep(&base ,";");
    if(token && (strcmp(token,"\0") > 0))/* se tiver ano imprime*/
    {
        mvwprintw(pwin->w[pwin->wi], (LINES/2) + 4 + j, 1 + tamanho, ", %s",token);
        tamanho += strlen(token) + 2;
    }
    token = strsep(&base ,";");
    if(token && (strcmp(token,"\0") > 0))/* se tiver Edicao imprime*/
    {
        mvwprintw(pwin->w[pwin->wi], (LINES/2) + 4 + j, 1 + tamanho, ", %s",token);
        tamanho += strlen(token) + 2;
    }
    if(base && (strcmp(token,"\0") > 0))/* se tiver ISBN imprime*/
        mvwprintw(pwin->w[pwin->wi], (LINES/2) + 4 + j, 1 + tamanho, ", %s",base);
...
void funcaoalterar(wins_t *pwin, char livro[626])
{
    ...
    /* Imprime pedido para alteracao segura na tela */
    mvwprintw(pwin->w[pwin->wi], 16, COLS/2 - 90 ,  "Reescreva os campos que nao deseja modificar e altere o que for desejavel.");
    wrefresh(pwin->w[pwin->wi]);
    refresh();
    ...
    while(1)
    {
        ...
        wrefresh(pwin->w[pwin->wi]);
        kpress = wgetch(pwin->w[pwin->wi]);
        switch(kpress)  /* lê o que o usuário pressiona */
        {
            case KEY_DOWN:  //desce as opções
            {
                contador ++;
        ...
        if(kpress == '\n')  // se pressionar enter abre o loop para escrever
        {
            if(contador == 0)   //se o usuário escolher autor
            {
                for(r = 0; r < 200; r++)
            ...
            if(contador == 1) //se o usuário escolher titulo
            {
                for(r = 0; r < 200; r++)
                {
                    titulo[r] = getch();
            ...
            if(contador == 2)   //se o usuário escolher editora
            {
                for(r = 0; r < 100; r++)
                {
                    editora[r] = getch();
            ...
            if(contador == 5) //se o usuário escolher edicao
            {
                for(r = 0; r < 10; r++)
                {
                    edicao[r] = getch();
        ...
        if(kpress == KEY_F(4)) //se o usuário apertar f4 ele cria a string que será alterada no arquivo livros.txt
        {

                if(autorC[1] != 0) /* esses ifs assim como na fun add, verificam se ha informacao na memoria */
        {
          o++;
        }
        if(tituloC[1] != 0)
        ...
        if(anoC[1] != 0)
        {
          o++;
                ...
                if(o>=5){                      /*caso as areas obrigatorias estajam preenchidas */
            strcat(dadosC, autorC);
            strcat(dadosC,";");
            strcat(dadosC,tituloC);
            strcat(dadosC,";");
        ...
        if(o<5)                      /*Caso nao estejam preenchidas ele volatara ao loop normalmente*/
        {
          o=0;
          if(mensagem_status(*pwin, "Ainda faltam informacoes obrigatorias, favor continue", STOK, "C" , NULL)){}
        }
   ...
   /*chama a função status para o usuário alterar confirmar as alterações*/

    escolha = mensagem_status(*pwin, "Confirma alteracao?", YESNO, "YES", "NO");
    if(escolha)
    {        /*abre os arquivos*/
        ...
        fclose(file);
        fclose(copia);
        rename("copia.txt","livros.txt"); // renomeia o arquivo criado
    }
    wrefresh(pwin->w[pwin->wi]);
...
void funcaodeletar(wins_t *pwin, char livro[626])
{
    ...
    /*chama a função status para o usuário deletar o livro*/
...
char *stristr(const char *String, const char *Pattern)
{
           ...
           start++, slen--)
      {
            ...
            while (toupper(*start) != toupper(*Pattern))
            {
                  ...
                  /* se o padrao for maior que a string */
